...
; todo jack: maybe just inline this
(defn filter-by-timestamps
  [timestamps data]
  (filter #(contains? timestamps (first %)) data))

; todo jack: unit tests + move it into util, perf opt?
(defn filter-parallel-data [data query]
  "Requires all query parameters to pass for a row to be returned. E.g.
  { 'Age' 25, 'sex' 'M' } is both 25 and male. Assumes data is a collection of parallel arrays, like:
...
; todo jack: combine the previous step into this one
  ...
  (mapv (fn [[k values]]
          (let [overall-risk (calculate-overall-risk values)]
            ...
            (if (zero? overall-risk)
              [k values]
              [k (re-weight-values overall-risk values)])))
        vectors))

; todo jack: look at the rounding code, I have a feeling it does the right thing already
; todo jack: the form validation is ass
; todo jack: unrelated note - :sub-text is probably what you want for the validation text
; todo jack: you can fix the input by doing the update on blur or something
; todo jack: note for merge request, we're sampling fewer points on the area chart (it used to be per quarter)
(defn create-visualization-context
  ...
  (let [day @(rf/subscribe [::subs/test-day])
        ...
        mortality-data (reformat-mortality-data (filter-parallel-data
        ...
        S0 (filter-by-timestamps (set sample-days) S0)
        all-S0 (filter-by-timestamps (set sample-days) all-S0)
        ...
        s0 all-S0
        ; todo jack: ensure the day is the same
        s0-for-day (model/S0-for-day s0 day)
        sum-betas (map #(fac/sum-beta-xs context %) beta-keys)
        F (normalize-vectors (merge-vectors (model/cox-only s0 sum-betas) mortality-data))
