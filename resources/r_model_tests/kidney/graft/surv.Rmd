...
g(b = 3, a = 4)
```
This allows us to construct a call to f with values obtained from an external table in a position independent form.

### Call adjcox
Use Belfast as the centre, calling adjcox with:
First All the zero effect levels - given by the defauk parameters
Then with the first non-zero test level, keeping all other levels at zero effect

```{r}
r <- adjcox(cent = "Belfast") %>% as.data.frame()

S <- function(day) {
  r[day+1,]$adjsurv
}
rbind(days, map_dbl(days, S))
```
...
rbind(days, map_dbl(days, R), map_dbl(days, W))
