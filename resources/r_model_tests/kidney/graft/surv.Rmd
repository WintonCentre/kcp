...
### Poke the changed forrmal parameters ino adjcox
```{r}
args <- formals(adjcox)
args[["rage"]] <- 1
...
So, let's call adjcox with the inputs given by some r\$zero and r\$test columns in factors:
```{r}
...
results <- 
  rbind(acc,
        results.at.times(NULL,NULL,days) %>% prepend("unchanged"),
        results.at.times("rage", 6, days) %>% prepend("rage"),
        results.at.times("wait", 5, days) %>% prepend("wait"))
results
```
How to make a list of results from factors?
```{r}
results.for.factor <- function (f) {
  results.at.times(f$r, f$test, days) %>% prepend(f$r)
}
# append.row <- function(rows, row) {
#   rows <- rbind(rows, row)
# }
# factor.rows <- by(factors, seq_len(nrow(factors)), function(row) {row %>% as.data.frame()}, simplify=TRUE)
acc <- array(NA, c(0,4))
for(i in seq_len(nrow(factors))) {
  acc <- rbind(acc, results.for.factor(factors[i,]))
}
acc %>% 
  as.data.frame() %>% rename(Var=V1, Year1 = V2, Year3 = V3, Year5=V4) %>%
  write.csv("test_results.csv", row.names=FALSE)
...
rename(df, Var=V1, Year1 = V2, Year3 = V3, Year5=V4)
```
Then with the first non-zero test levels, keeping all other levels at zero effect
```{r}
r <- adjcox(cent = "Belfast", rage = 6) %>% as.data.frame()
