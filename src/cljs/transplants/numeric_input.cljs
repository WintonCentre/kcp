...
(defn inc-dec-button 
  [{:keys [value-f increment onChange min max nmin nmax precision]
    :as   props}]
  (let [value (str-to-num (value-f))]
    [:span {:class-name "incdec"}
     [:button {:class-name  (str (if (pos? increment) "right" "left") " btn btn-default ")
               ...
               :tab-index   -1
               :on-click    #(update-value value nmin nmax precision increment onChange)}
      (if (pos? increment) "+" "–")]]))



(defn numeric-input 
  [{:keys [key value-f onChange min max error-color color precision] :or {error-color "red" color "black"} :as props}]

  (println "numeric-value =" (value-f))
    ...
    [:div {:class       "numeric-input"
           ...
           :on-key-down #(let [key-code (.. % -nativeEvent -code)]
                           ...
                           (update-value value nmin nmax precision
                                         ...
                                         onChange))}
     [:div
      #_(inc-dec-button (assoc props :nmin nmin :nmax nmax :precision precision :increment -1 :value-f value-f))
      [:input
       {:type      "text"
        :value     good
        :id        key
        ...
        :style     {:width            "58px" :height "36px" :font-size "14px"
                    ...
                    :background-color (if (js/isNaN value) "#dddddd" "#007bff")
                    :color            (if (nil? bad) "#fff" error-color)
                    :padding          "0 0 4px 0"
                    :text-align       "center"
