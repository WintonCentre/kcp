...
results <- 
  rbind(results.at.times(NULL,NULL,days) %>% prepend("unchanged"),
...
# rows <- data_frame()
# append.row <- function(row) {
#   str(row)
#   rows <- rbind(rows, row)
# }
# for(i in seq_len(nrow(factors))) {rows <- rbind(rows, factors[[,i]])}
#by(factors, seq_len(nrow(factors)), append.row)
