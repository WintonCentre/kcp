...
; todo jack: the problem is that (among other things probably) levels has a nil in it for age
(defn master-f-map
  ...
  (let [f-map (first f-maps)
        ...
        levels  (map (fn [[k [v]]] [k v])                   ; todo jack: this is wrong I think
