...
(defn bar-chart-graphic
  ...
  [:text {:x 0 :y 0 :font-size 30} (str (pr-str cifs-by-year))]
  [:g {:key 1}
   [:rect {:key        1
...
(defn bar-chart
  ...
  (let [{:keys [from-year to-year]} @(rf/subscribe [::subs/cohort-dates])
        ...
        outcomes (model/with-all-reasons-first
                   (fac/get-outcomes* (first baseline-cifs)))
        #_#_outcomes (if (= tool :waiting)
                   [(second outcomes)]
                   outcomes)
        beta-keys (fac/prefix-outcomes-keys outcomes "beta")
        outcome-keys (fac/prefix-outcomes-keys outcomes "cif")
        #_#_outcome-keys (if (= tool :waiting)
                       [(second outcome-keys)]
                       outcome-keys)
        sum-betas (map #(fac/sum-beta-xs env %) beta-keys)
        sample-days (map
        ...
        cifs-by-year #_(cifs-data {:bundle bundle
                                 :tool tool
                                 :outcome-keys outcome-keys #_(if (= tool :waiting)
                                                                [(second outcome-keys)]
                                                                outcome-keys)
                                 :outcomes outcomes #_(if (= tool :waiting)
                                             [(second outcomes)]
                                             outcomes)
                                 :sum-betas sum-betas
